...
function isStringStartOfWord(wordParam, stringParam) {
    if (wordParam[0] === stringParam[0] && wordParam[1] === stringParam[1] && wordParam[2] === stringParam[2]) {
        console.log(true);
    } else {
        console.log(false);
    }
...
isStringStartOfWord("poo", "pool for swimming");
isStringStartOfWord("poo", "poland");
isStringStartOfWord("poo", "good morning");
isStringStartOfWord("hello", "hell is very hot");
...
function calcLifetimeSupply(age, amoutPerDay) {
    for (let i = 0; i < 101; i++) {
        let increasingAge = age + age[i];
    }
    console.log(age * amoutPerDay);
}
